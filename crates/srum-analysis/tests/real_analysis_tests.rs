...
fn chainsaw_idmap_contains_nbtscan_in_tmp() {
    ...
    assert!(found, "chainsaw idmap must contain nbtscan.exe in a TMP path (APTSimulator discovery tool)");
}
...
fn chainsaw_idmap_contains_aptsimulator_curl_path() {
    ...
    let entries = parse_id_map(Path::new(CHAINSAW)).expect("parse_id_map");
    let found = entries.iter().any(|e| {
        e.name.to_lowercase().contains("aptsimulator")
    });
    assert!(found, "chainsaw idmap must contain a path with 'aptsimulator' (curl.exe helper dir)");
}
...
fn chainsaw_idmap_contains_renamed_psexec() {
    ...
    assert!(found, "chainsaw idmap must contain p.exe (PsExec renamed by APTSimulator)");
}
...
fn chainsaw_idmap_contains_procdump() {
    ...
    if !fixture_exists(CHAINSAW) { return; }
    let entries = parse_id_map(Path::new(CHAINSAW)).expect("parse_id_map");
    let found = entries.iter().any(|e| e.name.to_lowercase().contains("procdump"));
    assert!(found, "chainsaw idmap must contain procdump64.exe (APTSimulator LSASS dumper)");
}
...
fn chainsaw_idmap_contains_eventcreate() {
    ...
    if !fixture_exists(CHAINSAW) { return; }
    let entries = parse_id_map(Path::new(CHAINSAW)).expect("parse_id_map");
    let found = entries.iter().any(|e| e.name.to_lowercase().contains("eventcreate"));
    assert!(found, "chainsaw idmap must contain eventcreate.exe (APTSimulator fake event log)");
}
...
fn chainsaw_nbtscan_network_record_has_nonzero_bytes_sent() {
    ...
    let nbtscan_ids: std::collections::HashSet<i32> = id_map.iter()
        .filter(|e| {
            let n = e.name.to_lowercase();
            n.contains("nbtscan") && (n.contains("\\tmp\\") || n.contains("/tmp/"))
        })
        .map(|e| e.id)
        .collect();
    assert!(!nbtscan_ids.is_empty(), "nbtscan id must be present in idmap");

    let net_records = parse_network_usage(Path::new(CHAINSAW)).expect("parse_network_usage");
    let found = net_records.iter().any(|r| {
        nbtscan_ids.contains(&r.app_id) && r.bytes_sent > 0
    });
    assert!(found, "chainsaw must have a network record for nbtscan (TMP) with bytes_sent > 0");
}
...
fn chainsaw_nslookup_network_record_present() {
    ...
    if !fixture_exists(CHAINSAW) { return; }
    let id_map = parse_id_map(Path::new(CHAINSAW)).unwrap_or_default();
    let nslookup_ids: std::collections::HashSet<i32> = id_map.iter()
        .filter(|e| e.name.to_lowercase().contains("nslookup"))
        .map(|e| e.id)
        .collect();
    assert!(!nslookup_ids.is_empty(), "nslookup id must be present in idmap");

    let net_records = parse_network_usage(Path::new(CHAINSAW)).expect("parse_network_usage");
    let found = net_records.iter().any(|r| nslookup_ids.contains(&r.app_id));
    assert!(found, "chainsaw must have a network record for nslookup (C2 DNS lookups)");
}
...
fn chainsaw_suspicious_path_fires_on_nbtscan_in_tmp() {
    ...
    let found = timeline.iter().any(|r| {
        let app = r.get("app_name").and_then(|v| v.as_str()).unwrap_or("").to_lowercase();
        let flagged = r.get("suspicious_path").and_then(|v| v.as_bool()).unwrap_or(false);
...
fn belkasoft_idmap_contains_thunderbird_in_program_files() {
    ...
    assert!(found, "belkasoft idmap must contain thunderbird.exe under Program Files");
}
...
fn belkasoft_idmap_contains_git_in_program_files() {
    ...
    assert!(found, "belkasoft idmap must contain git.exe under Program Files");
}
...
fn belkasoft_idmap_contains_vscodium_in_user_documents() {
    ...
    assert!(found, "belkasoft idmap must contain VSCodium.exe under user Documents folder");
}
...
fn belkasoft_idmap_contains_user_anit_ghosh() {
    ...
    if !fixture_exists(BELKASOFT) { return; }
    let entries = parse_id_map(Path::new(BELKASOFT)).expect("parse_id_map");
    let found = entries.iter().any(|e| e.name.to_lowercase().contains("anit"));
    assert!(found, "belkasoft idmap must contain the primary user anit.ghosh");
}
...
fn belkasoft_idmap_has_no_apt_tools() {
    ...
    if !fixture_exists(BELKASOFT) { return; }
    let entries = parse_id_map(Path::new(BELKASOFT)).expect("parse_id_map");
    let apt_tools = ["nbtscan", "procdump", "mim.exe", "p.exe", "psexesvc", "eventcreate"];
    for tool in &apt_tools {
        let found = entries.iter().any(|e| e.name.to_lowercase().contains(tool));
        assert!(!found, "belkasoft idmap must not contain {tool} (clean machine, no APT tools)");
    }
}
